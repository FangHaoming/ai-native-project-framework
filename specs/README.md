...
npm run spec:transition -- <spec-filename> draft --actor "reviewer" --reason "Plan revision required"
npm run spec:transition -- <spec-filename> done --actor "reviewer" --reason "Review passed"
```

The CLI accepts exact filenames with or without `.md`, rejects ambiguous references and illegal transitions, prevents target overwrite, updates `Delivery Status`, and appends Transition History. Actor metadata is declarative local audit context, not authenticated identity.
